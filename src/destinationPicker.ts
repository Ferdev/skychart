...
export {
  DESTINATION_PICKER_STORAGE_KEY,
  clearRecentDestinations,
  getFrequentDestinationKeys,
  getRecentDestinationKeys,
  loadRecentDestinations,
  readRecentDestinations,
  recordRecentDestination,
  saveRecentDestinations,
  writeRecentDestinations,
  type DestinationPickerStorage,
  type RecentDestination,
...
function compareNullableDateDesc(left: string | null, right: string | null): number {
  if (left && right) return compareRecentDateDesc(left, right);
  if (left) return -1;
  if (right) return 1;
  return 0;
}
