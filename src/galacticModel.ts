...
export type GalacticProjectionCaveat = {
  key: string;
  label: string;
  detail: string;
};
...
export type GalacticModel = {
  ...
  projection: {
    frame: "galactocentric-j2000-ecliptic";
    caveats: GalacticProjectionCaveat[];
  };
  bounds: {
    minXAu: number;
    maxXAu: number;
  ...
  return {
    ...
    projection: {
      ...
      caveats: [
        {
          key: "schematic-arms",
          label: "Schematic spiral arms",
          detail: "Arm traces and dust lanes are contextual guides, not selectable catalog stars."
        },
        {
          key: "ecliptic-projection",
          label: "Projected into the atlas plane",
          detail: "The model starts in a galactocentric frame and is rotated into the same J2000 ecliptic plane used by catalog points."
        },
        {
          key: "retained-depth",
          label: "Retained depth coordinate",
          detail: "Model points retain x/y/z AU coordinates even though this map displays the projected x/y plane."
        }
      ]
    },
