...
catalog_tile_s3_prefix="${CATALOG_TILE_S3_PREFIX:-catalog-tiles/v1}"
...
nice -n "$nice_level" "$aws_cmd" \
  --endpoint-url "$CATALOG_TILE_S3_ENDPOINT_URL" \
  s3 cp "$catalog_tile_output/manifest.json" "s3://$CATALOG_TILE_S3_BUCKET/$catalog_tile_s3_prefix/manifest.json" \
