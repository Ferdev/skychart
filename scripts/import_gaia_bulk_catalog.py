...
DEFAULT_POSITION_EPOCH = float(os.environ.get("CATALOG_STELLAR_POSITION_EPOCH", "2026.0"))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--position-epoch", type=float, default=DEFAULT_POSITION_EPOCH)
...
def propagated_catalog_position(
    ra_deg: float,
    dec_deg: float,
    ...
    if pmra_mas_yr is None or pmdec_mas_yr is None:
        return (
            ra_deg,
            dec_deg,
            "No complete proper-motion vector was available; position remains at the Gaia DR3 source epoch.",
            GAIA_DR3_EPOCH,
        )

    years = target_epoch - GAIA_DR3_EPOCH
    cos_dec = max(math.cos(math.radians(dec_deg)), 1.0e-8)
    ...
    return (
        propagated_ra,
        propagated_dec,
        "RA/Dec propagated from Gaia DR3 epoch using catalog proper motion.",
        target_epoch,
    )
...
def copy_row(row: dict[str, str], args: argparse.Namespace, now: str, now_z: str) -> list[Any] | None:
    ...
    ra_deg, dec_deg, proper_motion_note, position_epoch = propagated_catalog_position(
        catalog_ra_deg,
        catalog_dec_deg,
        pmra_mas_yr,
        pmdec_mas_yr,
    ...
    facts = reject_none(
        {
            ...
            "proper_motion_note": proper_motion_note,
    ...
    return [
        ...
        args.group,
        "gaia_dr3",
        f"gaia_dr3_epoch_{position_epoch:.2f}".replace(".", "_")
        + ("_proper_motion_coordinates" if position_epoch != GAIA_DR3_EPOCH else "_catalog_coordinates"),
        None,
