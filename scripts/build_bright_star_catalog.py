...
SOURCE_QUERY = {
    "-source": SOURCE_TABLE,
    "-out": "HIP,HD,RAICRS,DEICRS,pmRA,pmDE,Plx,Vmag,B-V,SpType",
    "Vmag": "<6.5",
    "Plx": ">0",
    "-out.max": "unlimited",
}
...
def propagated_catalog_position(
    ...
    target_epoch: float = DEFAULT_POSITION_EPOCH,
) -> tuple[float, float, str, float]:
    if pmra_mas_yr is None or pmdec_mas_yr is None:
        return (
            ra_deg,
            dec_deg,
            "No complete proper-motion vector was available; position remains at the Hipparcos source epoch.",
            HIPPARCOS_EPOCH,
        )

    years = target_epoch - HIPPARCOS_EPOCH
    ...
    return (
        propagated_ra,
        propagated_dec,
        "RA/Dec propagated from Hipparcos epoch using catalog proper motion.",
        target_epoch,
    )
...
def build_stars(rows: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        pmdec_mas_yr = finite_float(row.get("pmDE"))
