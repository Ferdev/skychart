...
SB_SAT_URL = "https://ssd-api.jpl.nasa.gov/sb_sat.api"
...
def fetch_satellite_json(params: dict[str, str]) -> dict[str, Any]:
    request = Request(
        f"{SB_SAT_URL}?{urlencode(params)}",
        headers={"User-Agent": "CosmicAtlasCatalogBuilder/1.0"},
    )
    with urlopen(request, timeout=120) as response:
        return json.loads(response.read().decode("utf-8"))
...
def fetch_rows() -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for adapter in SOURCE_ADAPTERS:
        if adapter["api"] == "sb_sat":
            rows, count, total_count = fetch_satellite_adapter_rows(adapter)
            for row in rows:
                key = str(row.get("spkid") or "").strip()
                if key:
                    by_spkid[key] = row
            slice_meta.append(
                {
        ...
        payload = fetch_json(adapter["params"])
        ...
        for values in payload.get("data", []):
            ...
            if existing:
                existing.setdefault("_selection", []).append(adapter["name"])
                existing.setdefault("_source_adapters", []).append(adapter["name"])
            else:
                row["_selection"] = [adapter["name"]]
                row["_source_adapters"] = [adapter["name"]]
                row["_source_family"] = adapter["family"]
                by_spkid[spkid] = row
            count += 1
...
def fetch_satellite_adapter_rows(adapter: dict[str, Any]) -> tuple[list[dict[str, Any]], int, Any]:
    payload = fetch_satellite_json(adapter["params"])
    rows = []

    for record in payload.get("data", []):
        row = satellite_record_to_small_body_row(record, adapter)
        if row is not None:
            rows.append(row)

    limit = finite_int(os.environ.get("SMALL_BODY_SATELLITE_LIMIT"))
    if limit is not None:
        rows = rows[:limit]

    return rows, len(rows), payload.get("count")


def satellite_record_to_small_body_row(record: dict[str, Any], adapter: dict[str, Any]) -> dict[str, Any] | None:
    sat = record.get("sat") or {}
    orbit = record.get("orbit") or {}
    phys_par = record.get("phys_par") or {}
    fullname = clean_full_name(sat.get("sat_fullname")) or clean_full_name(sat.get("iau_name"))
    primary_designation = clean_full_name(sat.get("pdes"))
    if not fullname or not primary_designation:
        return None

    key = f"jpl-sb-sat-{slugify(primary_designation)}-{slugify(fullname)}"
    diameter = phys_value(phys_par, "diameter")
    semi_major_axis_km = finite_float(orbit.get("a"))
    semi_major_axis_au = semi_major_axis_km / AU_KM if semi_major_axis_km is not None else None

    return {
        "spkid": key,
        "full_name": fullname,
        "name": clean_full_name(sat.get("iau_name")) or fullname,
        "kind": "satellite",
        "class": sat.get("class"),
        "diameter": diameter,
        "rot_per": phys_value(phys_par, "rot_per"),
        "GM": phys_value(phys_par, "GM"),
        "epoch": orbit.get("epoch"),
        "e": orbit.get("e"),
        "a": semi_major_axis_au,
        "q": finite_float(orbit.get("q")) / AU_KM if finite_float(orbit.get("q")) is not None else None,
        "i": orbit.get("i"),
        "om": orbit.get("om"),
        "w": orbit.get("w"),
        "ma": orbit.get("ma") or 0.0,
        "n": orbit.get("n") or mean_motion_from_period_hours(orbit.get("per")),
        "_selection": [adapter["name"]],
        "_source_adapters": [adapter["name"]],
        "_source_family": adapter["family"],
        "_parent_designation": primary_designation,
        "_satellite_record": reject_none(record),
    }


def phys_value(phys_par: dict[str, Any], key: str) -> float | None:
    item = phys_par.get(key)
    if isinstance(item, dict):
        return finite_float(item.get("value"))
    return None


def mean_motion_from_period_hours(value: Any) -> float | None:
    period_hours = finite_float(value)
    if period_hours is None or period_hours <= 0:
        return None
    return 360.0 / (period_hours / 24.0)
...
def build_object(row: dict[str, Any], target_jd: float) -> dict[str, Any] | None:
    ...
    key = spkid if spkid.startswith("jpl-sb-sat-") else f"jpl-sbdb-{spkid}"
    parent_designation = clean_full_name(row.get("_parent_designation"))
    parent_key = f"jpl-small-body-{slugify(parent_designation)}" if parent_designation else "sun"
    source_type = "jpl_sb_sat" if object_type == "satellite" else "jpl_sbdb_query"
    position_model = (
        "jpl_sb_sat_primary_relative_two_body_elements"
        if object_type == "satellite"
        else "jpl_sbdb_two_body_osculating_elements"
    )
    ...
    return {
        ...
        "external_ids": {
            ...
            "small_body_satellite_designation": full_name if object_type == "satellite" else None,
        },
        "facts": {
            "full_name": full_name,
            ...
            "source_adapters": row.get("_source_adapters", []),
...
def interesting_note(row: dict[str, Any], object_type: str) -> str:
    ...
    if object_type == "satellite":
        return "Natural satellite subset from the NASA/JPL Small-Body Database."
    if row.get("pha") == "Y":
...
def main() -> None:
    ...
    payload = {
        ...
        "source": {
            ...
            "satellite_api_url": SB_SAT_URL,
            "documentation_url": SBDB_DOC_URL,
            "filter_documentation_url": SBDB_FILTER_DOC_URL,
            "satellite_documentation_url": SB_SAT_DOC_URL,
            "fields": FIELDS,
            "source_adapters": slice_meta,
        },
        "selection": {
            "description": "Union of explicit asteroid, comet, and natural-satellite SBDB source adapters.",
