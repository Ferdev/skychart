...
def distance_fields(row: dict[str, str]) -> dict[str, object]:
    parallax_mas = parse_float(row.get("Pax"))
    redshift = parse_float(row.get("Redshift"))
    radial_velocity = parse_float(row.get("RadVel"))
    ...
    if radial_velocity and radial_velocity > 0:
        distance_mpc = radial_velocity / HUBBLE_CONSTANT_KM_S_MPC
        return {
            "distance_pc": round(distance_mpc * 1_000_000.0, 8),
            "distance_ly": round(distance_mpc * 3_261_563.777, 8),
            "distance_quality": "hubble_flow_radial_velocity_approximation",
        }
...
def build_object(row: dict[str, str]) -> dict[str, object] | None:
    ...
    obj.update(distance_fields(row))
...
def build_catalog(source_csv: str) -> dict[str, object]:
    ...
    return {
        ...
        "source": {
            ...
            "commit": commit,
            "used_for": "NGC/IC designations, RA/Dec, object classes, constellation, magnitudes, angular sizes, aliases, common names, and redshift/radial-velocity distance estimates where available",
        },
        "selection": {
            "included": "NGC and IC entries with coordinates and physical/catalog object types",
            "excluded_types": sorted(SKIPPED_TYPES),
            "distance_note": "Distances are recorded only when OpenNGC supplies positive parallax, redshift, or radial velocity; redshift/radial-velocity distances use a simple Hubble-flow approximation with H0=70 km/s/Mpc.",
        },
        "objects": objects,
    }
