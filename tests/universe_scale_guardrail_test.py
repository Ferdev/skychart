...
class UniverseScaleGuardrailTest(unittest.TestCase):
    def test_catalog_points_have_no_minimum_physical_view_width(self) -> None:
        self.assertNotIn("POINT_LAYER_MIN_WIDTH_LY", MAIN)
        body = MAIN[MAIN.index("function shouldUseCatalogPoints") : MAIN.index("function hasActiveCatalogPointLayer")]
    ...
    def test_catalog_point_layers_reach_cosmological_scale(self) -> None:
        self.assertIn("POINT_LAYER_DEEP_SKY_MAX_WIDTH_LY", MAIN)
        self.assertIn("POINT_LAYER_QUASAR_MAX_WIDTH_LY", MAIN)
        self.assertIn("POINT_TILE_MAX_ACTIVE_UNIVERSE", MAIN)
        self.assertIn("POINT_TILE_MAX_POINTS_UNIVERSE", MAIN)
    ...
    def test_desi_groups_are_available_in_deep_sky_filters(self) -> None:
        self.assertGreaterEqual(MAIN.count('"desi_dr1_galaxies"'), 3)
        self.assertGreaterEqual(MAIN.count('"desi_dr1_quasars"'), 3)
        self.assertIn("hydrateDesiCatalogTilePoint", MAIN)
        self.assertIn("desi_targetid", MAIN)
    ...
    def test_smp3_load_prefix_is_stable_and_hit_testing_is_dense(self) -> None:
        request_body = MAIN[MAIN.index("function catalogPointRequestsForLevel") : MAIN.index("function prioritizeCatalogPointRequests")]
        hit_body = MAIN[MAIN.index("function rebuildCatalogPointHitGrid") : MAIN.index("function rebuildBodyHitGrid")]
        self.assertNotIn("adaptiveCatalogPointBudget", MAIN)
        self.assertIn("? configuredLimit", request_body)
        self.assertNotIn("payload.returned / 2_500", hit_body)
        self.assertIn("CATALOG_POINT_HIT_GRID_CELL_PX", hit_body)
        self.assertIn('requiredElement<HTMLElement>("#catalog-point-hover")', MAIN)
