defmodule StarsmapApiWeb.SitemapController do
  ...
  def catalog(conn, %{"catalog" => filename}) do
    ...
    if filename == group <> ".xml" and Regex.match?(~r/\A[a-z0-9_-]{1,80}\z/, group) and
         Enum.any?(Catalog.sitemap_catalogs(), fn {name, _, _} -> name == group end) do
  ...
  defp index_xml do
    ...
    entries =
      Enum.map_join(Catalog.sitemap_catalogs(), "", fn {group, _, lastmod} ->
  ...
  defp catalog_xml(group) do
    ...
    entries =
      Enum.map_join(Catalog.sitemap_entries(group), "", fn {key, lastmod} ->
